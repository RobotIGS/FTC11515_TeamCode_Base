...
public class Gegensteuern {
    ...
    public double calculate(boolean geg, double steuer_alt, double steuer_neu) {
        ...
        if (absNeu > JOYSTICK_TOLERANZ) {
            // Abbruch: Fahrer steuert wieder aktiv selbst
            bremszeitEnde = 0;
            basisBremsKraft = 0;
            endwert = steuer_neu;
        } else if (Math.abs(steuer_alt) > JOYSTICK_TOLERANZ && bremszeitEnde <= now) {
            // Aktivierung: Stick wurde gerade losgelassen und Bremsung läuft noch nicht
            // in Millisekunden

            aktuelleBremsDauer = Math.round(MAX_BREMSZEIT * Math.abs(durchschnitt));
